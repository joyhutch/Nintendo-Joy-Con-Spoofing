...
class L2CAP_Transport(asyncio.Transport):
    ...
    async def _reader(self):
        while True:
            await self._protocol.report_received(
                await self.read(), self._itr_sock.getpeername()
            )
    ...
    async def read(self):
        ...
        logger.debug(f'received "{str(hexlify(data))}"')

        if not data:
            # disconnect happened
            logger.error("No data received." + str(data))
            self._protocol.connection_lost(None)  # TODO: unclear if this helps or not
    ...
    async def write(self, data: Any) -> None:
        ...
        logger.debug(f'sending "{str(hexlify(_bytes))}"')
